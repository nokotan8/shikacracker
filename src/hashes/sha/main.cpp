...
bool compute_hash_openssl(const char *input, size_t input_len, std::string &hashed_str,
...
void compute_sha256(const char *input, const size_t input_len, char *output) {
    sha256_context ctx;
    sha256_init(&ctx);
...
int main(void) {
    ...
    for (size_t i = 1; i < MAX_LEN; i++) {
        for (size_t j = 0; j < EACH_NUM; j++) {
            ...
            compute_hash(input, i, openssl_output, EVP_sha256());
            compute_sha256(input, i, my_output);
